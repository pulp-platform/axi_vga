...
package tb_axi_vga_pkg;
	...
	localparam axi_vga_reg_pkg::axi_vga_reg2hw_t cfg = '{
		...
		frame_size:            (PixelWidth/8 * FrameWidth * FrameHeight),
		burst_len:             32'h000000FF,
		burst_split_len:       32'h00000007
	};

	localparam int unsigned FullRenderWidth = ( cfg.hori_visible_size + cfg.hori_front_porch_size +
												cfg.hori_sync_size + cfg.hori_back_porch_size );
	localparam int unsigned FullRenderHeight = ( cfg.vert_visible_size + cfg.vert_front_porch_size +
												cfg.vert_sync_size + cfg.vert_back_porch_size );
